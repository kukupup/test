...
public class OmsOrderServiceImpl extends ServiceImpl<OmsOrderMapper, OmsOrder> implements OmsOrderService {
    ...
    public ConfirmOrderDTO generateConfirmOrder(List<Long> ids) {
        ...
        List<OmsCartItem> omsCartItemsList=null;
//        List<Long> iids= null;
        if(CollectionUtil.isEmpty(ids)){
            throw new ApiException(ResultCode.VALIDATE_FAILED);
        }
        ConfirmOrderDTO confirmOrderDTO = new ConfirmOrderDTO();
//        if(orderParamDTO.getProductId()!=null) {
//            //直接购买需要添加的购物车信息
//            AddCarDTO AddCarDTO = new AddCarDTO();
//
//            List<Long> productSkuIdList= orderParamDTO.getProductSkuId();
//            List<Long> productIdList=  orderParamDTO.getProductId();
//
//            for (Long  productSkuId: productSkuIdList) {
//                AddCarDTO.setProductSkuId(productSkuId);
//            }
//            for (Long  productId : productIdList) {
//                AddCarDTO.setProductId(productId);
//            }
//            AddCarDTO.setQuantity(orderParamDTO.getQuantity());
//
//            System.out.println("ProductId"+AddCarDTO.getProductId()+"skuid"+AddCarDTO.getProductSkuId()+"数量"+AddCarDTO.getQuantity());
//            Integer result = cartItemService.add(AddCarDTO);
//            System.out.println("是否插入购物车成功");
//            if (result==null) {
//                throw new ApiException(ResultCode.FAILED);
//            }
//            System.out.println("返回的result的值"+result);
//
//// 1.商品
//             iids.add(result.longValue());
//            omsCartItemsList  = cartItemService.listByIds(iids);
//        }else{
            omsCartItemsList=cartItemService.listByIds(ids);
//        }
    ...
    public void cancelOverTimeOrder() {
        ...
        // 获取当前时间的指定时间之前 根据overtime定如果，创建时间是12点，那么这个是11点50分 ，才会10分钟过期
    ...
    public IPage<OrderListDTO> getMyOrders(Integer pageSize, Integer pageNum) {
        ...
        //这里如果使用嵌套结果查询关联其他表，分页会按照主表order的总条数去分页，会少分页数（mybatis的问题），但用嵌套查询，还是以第一个查询order表为分页，在查询其详情订单
        //执行分页，还要嵌套结果，会少数据，用分页用嵌套查询了
...
//        }
    ...
    private void lockStock(List<CartItemStockDTO> cartItemStockByIds) {
        //遍历购物车库存id，锁定，更新字段，真实库存显示是要减去锁定库存的
    ...
    public String getOrderSn(OmsOrder omsOrder){
        ...
        //插入到redis，根据后面delta值自动增加
        //portal:orderId:20210913 里面的value是自增的每次增加1
